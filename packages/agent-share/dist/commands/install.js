...
import { selectArtifactTypes, selectItemsWithExistenceCheck, selectTargets, confirmAction } from '../utils/prompts.js';
    ...
    try {
        ...
        for (const type of types) {
            if (available[type]?.length > 0) {
                const targetDirs = targetConfigs.map(tc => path.join(cwd, tc.config[type]));
                const result = await selectItemsWithExistenceCheck(available[type], type, {
                    mode,
                    targetDirs,
        ...
        for (const { key: targetKey, config: targetConfig } of targetConfigs) {
            ...
            for (const [type, items] of Object.entries(selected)) {
                for (const item of items) {
                    const targetDir = path.join(cwd, targetConfig[type]);
        ...
        const summaryLines = summaries.map(s => {
            const parts = [];
            if (s.installed > 0)
                parts.push(chalk.green(`${s.installed} installed`));
            if (s.skipped > 0)
                parts.push(chalk.yellow(`${s.skipped} skipped`));
            if (s.failed > 0)
                parts.push(chalk.red(`${s.failed} failed`));
